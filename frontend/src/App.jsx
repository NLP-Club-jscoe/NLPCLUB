...
const ScrollVideoFrames = () => {
  ...
  const preloadImages = useCallback(async () => {
    ...
    for (let i = 1; i <= totalFrames; i++) {
      ...
      darkImg.src = `/frames/dark/ezgif-frame-${frameNumber}.jpg`;
      ...
      lightImg.src = `/frames/light/ezgif-frame-${frameNumber}.jpg`;
  ...
  const themeValues = useMemo(() => ({
    ...
    bgColor: isDarkMode ? 'bg-black' : 'bg-white',
    heroBgColor: isDarkMode ? 'bg-black' : 'bg-[#A3ADBA]',
    textColor: isDarkMode ? 'text-white' : 'text-black',
    navBg: isDarkMode ? 'bg-black/20 border-white/10' : 'bg-white/20 border-black/10',
    buttonBg: isDarkMode ? 'bg-white text-black hover:bg-gray-100' : 'bg-black text-white hover:bg-gray-800',
    spinnerColor: isDarkMode ? 'border-white' : 'border-black',
    gradientColors: isDarkMode ? 'from-blue-400 to-purple-400' : 'from-blue-600 to-purple-600',
    textStroke: isDarkMode ? '1px rgba(255,255,255,0.1)' : '1px rgba(0,0,0,0.1)',
    textShadow: isDarkMode ? '2px 2px 4px rgba(0,0,0,0.8)' : '2px 2px 4px rgba(255,255,255,0.8)',
  ...
  const textAnimations = useMemo(() => {
    ...
    if (currentFrame >= textVisibilityStart && currentFrame <= textVisibilityEnd) {
      const progress = (currentFrame - textVisibilityStart) / textVisibilityRange;
      textOpacity = Math.min(1, progress * 2);
      textTransform = `translateX(${-100 + (progress * 100)}px)`;
    } else if (currentFrame > textVisibilityEnd) {
      const fadeOutProgress = Math.min(1, (currentFrame - textVisibilityEnd) / 10);
  ...
  return (
    <div className={`w-full ${themeValues.bgColor} transition-colors duration-300`}>
      {/* Glass Navigation Bar - Sticky */}
      <nav className={`fixed top-0 left-0 right-0 z-40 backdrop-blur-md border-b transition-all duration-300 ${themeValues.navBg}`}>
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            {/* Logo/Brand */}
            <div className={`text-xl font-bold transition-colors duration-300 ${themeValues.textColor}`}>
              NLP CLUB
            </div>
            ...
            <div className="hidden md:flex items-center justify-center space-x-8 flex-1">
              <a 
                ...
                className={`transition-all duration-300 hover:scale-110 ${
                  isDarkMode ? 'text-white/90 hover:text-white' : 'text-black/90 hover:text-black'
                }`}
              >
                Home
              </a>
            ...
            {/* Mobile Menu Button */}
            <button className={`md:hidden p-2 ${themeValues.textColor}`}>
            ...
            <>
              ...
              <div className="w-full h-full flex relative">
                ...
                <div className="w-2/4 relative">
                  {!isDarkMode && (
                    <div className={`absolute inset-0 ${themeValues.glassGradient}`}></div>
                  )}
                </div>
                ...
                <div className="w-2/4 h-full relative">
                  ...
                  {!isDarkMode && (
                    <div className={`absolute inset-0 ${themeValues.imageOverlay}`}></div>
                  )}
                </div>
                ...
                {!isDarkMode && (
                  <>
                    <div className="absolute left-1/2 top-0 bottom-0 w-32 -translate-x-1/2 bg-gradient-to-r from-transparent via-white/10 to-transparent backdrop-blur-md pointer-events-none"></div>
                    
                    {/* Additional subtle blend overlay */}
                    <div className="absolute left-1/2 top-0 bottom-0 w-16 -translate-x-1/2 bg-gradient-to-r from-[#A3ADBA]/30 via-transparent to-[#A3ADBA]/30 pointer-events-none"></div>
                  </>
                )}
              </div>
              ...
              <div className="absolute inset-0 flex items-center pointer-events-none">
                <div className="pl-8 md:pl-16 ml-8 md:ml-12 relative z-10">
                  ...
                  >
                    <div className="text-3xl md:text-5xl lg:text-6xl">Welcome To</div>
                    <span 
              ...
              >
                <div className={`text-xl md:text-2xl font-semibold ${
                  ...
                  <div className="backdrop-blur-sm bg-black/20 px-4 py-2 rounded-lg">
                    Discover the future of Natural Language Processing
                  </div>
                </div>
              </div>
            </>
